...
'
Public Const PCAN_ERROR_OK           As Long = &H00000  ' No error 
Public Const PCAN_ERROR_XMTFULL      As Long = &H00001  ' Transmit buffer in CAN controller is full
Public Const PCAN_ERROR_OVERRUN      As Long = &H00002  ' CAN controller was read too late
Public Const PCAN_ERROR_BUSLIGHT     As Long = &H00004  ' Bus error: an error counter reached the 'light' limit
Public Const PCAN_ERROR_BUSHEAVY     As Long = &H00008  ' Bus error: an error counter reached the 'heavy' limit
Public Const PCAN_ERROR_BUSOFF       As Long = &H00010  ' Bus error: the CAN controller is in bus-off state
...
Public Const PCAN_ERROR_QRCVEMPTY    As Long = &H00020  ' Receive queue is empty
Public Const PCAN_ERROR_QOVERRUN     As Long = &H00040  ' Receive queue was read too late
Public Const PCAN_ERROR_QXMTFULL     As Long = &H00080  ' Transmit queue is full
Public Const PCAN_ERROR_REGTEST      As Long = &H00100  ' Test of the CAN controller hardware registers failed (no hardware found)
Public Const PCAN_ERROR_NODRIVER     As Long = &H00200  ' Driver not loaded
Public Const PCAN_ERROR_HWINUSE      As Long = &H00400  ' Hardware already in use by a Net
Public Const PCAN_ERROR_NETINUSE     As Long = &H00800  ' A Client is already connected to the Net
Public Const PCAN_ERROR_ILLHW        As Long = &H01400  ' Hardware handle is invalid
Public Const PCAN_ERROR_ILLNET       As Long = &H01800  ' Net handle is invalid
Public Const PCAN_ERROR_ILLCLIENT    As Long = &H01C00  ' Client handle is invalid
...
Public Const PCAN_ERROR_RESOURCE     As Long = &H02000  ' Resource (FIFO, Client, timeout) cannot be created
Public Const PCAN_ERROR_ILLPARAMTYPE As Long = &H04000  ' Invalid parameter
Public Const PCAN_ERROR_ILLPARAMVAL  As Long = &H08000  ' Invalid parameter value
...
Public Const PCAN_PEAKCAN   = &H01  ' PCAN Non-Plug&Play devices. NOT USED WITHIN PCAN-Basic API
Public Const PCAN_ISA       = &H02  ' PCAN-ISA, PCAN-PC/104, and PCAN-PC/104-Plus
Public Const PCAN_DNG       = &H03  ' PCAN-Dongle
Public Const PCAN_PCI       = &H04  ' PCAN-PCI, PCAN-cPCI, PCAN-miniPCI, and PCAN-PCI Express
Public Const PCAN_USB       = &H05  ' PCAN-USB and PCAN-USB Pro
Public Const PCAN_PCC       = &H06  ' PCAN-PC Card

' PCAN parameters
'
Public Const PCAN_DEVICE_NUMBER       = &H01  ' PCAN-USB device number parameter
Public Const PCAN_5VOLTS_POWER        = &H02  ' PCAN-PC Card 5-Volt power parameter
Public Const PCAN_RECEIVE_EVENT       = &H03  ' PCAN receive event handler parameter
Public Const PCAN_MESSAGE_FILTER      = &H04  ' PCAN message filter parameter
Public Const PCAN_API_VERSION         = &H05  ' PCAN-Basic API version parameter
Public Const PCAN_CHANNEL_VERSION     = &H06  ' PCAN device channel version parameter
Public Const PCAN_BUSOFF_AUTORESET    = &H07  ' PCAN Reset-On-Busoff parameter
Public Const PCAN_LISTEN_ONLY         = &H08  ' PCAN Listen-Only parameter
Public Const PCAN_LOG_LOCATION        = &H09  ' Directory path for trace files
Public Const PCAN_LOG_STATUS          = &H0A  ' Debug-Trace activation status
Public Const PCAN_LOG_CONFIGURE       = &H0B  ' Configuration of the debugged information (LOG_FUNCTION_***)
Public Const PCAN_LOG_TEXT            = &H0C  ' Custom insertion of text into the log file
...
'
Public Const PCAN_PARAMETER_OFF       = &H00  ' The PCAN parameter is not set (inactive)
Public Const PCAN_PARAMETER_ON        = &H01  ' The PCAN parameter is set (active)
Public Const PCAN_FILTER_CLOSE        = &H00  ' The PCAN filter is closed. No messages will be received
Public Const PCAN_FILTER_OPEN         = &H01  ' The PCAN filter is fully opened. All messages will be received
Public Const PCAN_FILTER_CUSTOM       = &H02  ' The PCAN filter is custom configured. Only registered messages will be received
...
Public Const LOG_FUNCTION_DEFAULT     = &H00  ' Logs system exceptions / errors
Public Const LOG_FUNCTION_ENTRY       = &H01  ' Logs the entries to the PCAN-Basic API functions 
Public Const LOG_FUNCTION_PARAMETERS  = &H02  ' Logs the parameters passed to the PCAN-Basic API functions 
Public Const LOG_FUNCTION_LEAVE       = &H04  ' Logs the exits from the PCAN-Basic API functions 
Public Const LOG_FUNCTION_WRITE       = &H08  ' Logs the CAN messages passed to the CAN_Write function
Public Const LOG_FUNCTION_READ        = &H10  ' Logs the CAN messages received within the CAN_Read function

' PCAN message types
'
Public Const PCAN_MESSAGE_STANDARD    = &H00  ' The PCAN message is a CAN Standard Frame (11-bit identifier)
Public Const PCAN_MESSAGE_RTR         = &H01  ' The PCAN message is a CAN Remote-Transfer-Request Frame
Public Const PCAN_MESSAGE_EXTENDED    = &H02  ' The PCAN message is a CAN Extended Frame (29-bit identifier)
...
'
Public Const PCAN_BAUD_1M             = &H0014  '   1 MBit/s
Public Const PCAN_BAUD_800K           = &H0016  ' 800 kBit/s
Public Const PCAN_BAUD_500K           = &H001C  ' 500 kBit/s
Public Const PCAN_BAUD_250K           = &H011C  ' 250 kBit/s
Public Const PCAN_BAUD_125K           = &H031C  ' 125 kBit/s
...
Public Const PCAN_TYPE_ISA            = &H01  ' PCAN-ISA 82C200
Public Const PCAN_TYPE_ISA_SJA        = &H09  ' PCAN-ISA SJA1000
Public Const PCAN_TYPE_ISA_PHYTEC     = &H04  ' PHYTEC ISA 
Public Const PCAN_TYPE_DNG            = &H02  ' PCAN-Dongle 82C200
Public Const PCAN_TYPE_DNG_EPP        = &H03  ' PCAN-Dongle EPP 82C200
Public Const PCAN_TYPE_DNG_SJA        = &H05  ' PCAN-Dongle SJA1000
Public Const PCAN_TYPE_DNG_SJA_EPP    = &H06  ' PCAN-Dongle EPP SJA1000
...
Public Declare Function CAN_Initialize Lib "PCANBasic.DLL" _
    ...
    ByVal HwType As Byte, _
    ByVal IOPort As Long, _
    ByVal Interrupt As Integer) As Long
