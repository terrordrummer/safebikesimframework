...
bool CP2P::CreateChannel(std::string channel_name) {
    ...
    if (pipe_handle == INVALID_HANDLE_VALUE) {
        ...
        FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, 
            ...
            1024,
            NULL);
        PushError(std::string("[Create] System error crating the channel: <") + std::string(msg) + std::string(">"));
        return false;
        }
    std::cout << "created " << pipe_name << std::endl;
    // pipe created
...
bool CP2P::WaitForConnections(std::string channel_name) {
    ...
    if (it == _channelMap.end()) {
        ...
        std::stringstream s;
        s << "[WaitForConnections] channel <" << channel_name << "> not found" ;
        PushError(s.str());
        return false;
        }
    // CHECK ERRORS
...
bool CP2P::Connect(std::string channel_name) {
    ...
    if (pipe_handle == INVALID_HANDLE_VALUE) {
        ...
        FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, 
            ...
            1024,
            NULL);
        PushError(std::string("[Connect] System error connecting the channel: <") + std::string(msg) + std::string(">"));
        return false;
        }
            ...
            if (winGetLastError() == ERROR_MORE_DATA) {
                ...
                read_pos += *data_readed;
                    ...
                    FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, 
                        ...
                        1024,
                        NULL);
                    PushError(std::string("[Read] System error reading the channel: <") + std::string(msg) + std::string(">"));
                    *data_readed = 0;
                    return false;
                    }
            } else
...
bool CP2P::Write(std::string channel_name, unsigned int size_to_write, void *data_content) {
    ...
    if (WriteFile(_channelMap[channel_name], data_content, size_to_write, (LPDWORD)&data_written, NULL) == NULL) {
        ...
        FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, 
            ...
            1024,
            NULL);
        PushError(std::string("[Write] System error writing the channel: <") + std::string(msg) + std::string(">"));
        return false;
        }
    // data written
    return true;
...
bool CP2P::Close(std::string channel_name) {
    ...
    if (it == _channelMap.end()) {
        // no channel to close
        std::stringstream s;
        s << "[Close] Attemting to close a non-existent channel <" << channel_name << ">";
        PushError(s.str());
        return false;
        }
